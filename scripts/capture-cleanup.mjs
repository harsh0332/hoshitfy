...
const targetDir = 'docs/revision-1/after-cleanup';
...
async function run() {
  ...
  console.log('All after-cleanup screenshots captured successfully!');
  process.exit(0);
}
...
async function captureFullPage(vp, targetDir) {
  ...
  // Scroll through page to force Chrome to rasterize and paint all offscreen sections
  console.log(`Rasterizing full page at ${vp.width}px...`);
  await send('Runtime.evaluate', {
    expression: `
      (async () => {
        const total = document.body.scrollHeight;
        for (let y = 0; y < total; y += 1000) {
          window.scrollTo(0, y);
